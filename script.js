...
function generatePaymentDayOptions() {
    ...
    for (let monthOffset = 0; monthOffset < 2 && optionsGenerated < 8; monthOffset++) {
        ...
        fixedDays.forEach(day => {
            ...
            // Cria a data em UTC para evitar problemas de fuso horário com o dia específico
            const paymentDateAttempt = new Date(Date.UTC(year, month, day));
            
            // Verifica se o dia do objeto Date é o mesmo que tentamos criar
            // Isso evita que, por exemplo, 30 de Fev (que não existe) seja tratado como 2 de Março.
            if (paymentDateAttempt.getUTCDate() !== day) {
                return; 
            }

            const minSelectableDate = new Date(Date.UTC(currentYearUTC, currentMonthUTC, currentDayUTC + 1)); // Pelo menos amanhã

            if (paymentDateAttempt.getTime() >= minSelectableDate.getTime()) {
                const dateStr = formatDateForInput(paymentDateAttempt);
                if (addedDates.has(dateStr)) return; // Evita datas duplicadas se o mês for curto
...
function formatDateForInput(date) { // Recebe objeto Date
...
function selectEntryOption(type, element) {
    ...
    if (type === 'custom') {
        ...
        // Se for custom e o valor bruto for 0, não recalcula ainda, espera input
        if (appData.entry.customAmountRaw > 0) {
            handleCustomEntryValueChange(); // Recalcula se já houver valor
        } else {
             appData.entry.calculatedAmount = 0; // Se não houver valor, zera para custom
             updateAllCalculationsAndUI(); // Atualiza UI mesmo se for 0
        }
    } else {
...
function calculatePMT(principal, numberOfPayments) {
    ...
    if (numberOfPayments === 3) {
        ...
        if (Math.pow(1 + monthlyRate, numberOfPayments) - 1 === 0) return principal / numberOfPayments; // Avoid division by zero if rate makes denominator zero
...
function prepareSummaryAndWhatsApp() {
    ...
    document.getElementById('summaryTotalProducts').textContent = appData.baseTotalAmount.toFixed(2);
    document.getElementById('summaryWarrantyType').textContent = appData.warranty.type === 'none' ? 'Nenhuma' : `${appData.warranty.type} meses`;
    
    document.getElementById('summaryEntryAmount').textContent = appData.entry.calculatedAmount.toFixed(2);
    document.getElementById('summaryEntryPercentage').textContent = (appData.entry.effectivePercentage * 100).toFixed(1);
    
    document.getElementById('summaryAmountFinancedInternal').textContent = appData.financing.amountForInstallmentCalculation.toFixed(2);
    
    document.getElementById('summaryInstallmentMonths').textContent = si.months;
    document.getElementById('summaryInstallmentValue').textContent = si.value.toFixed(2);
    document.getElementById('summaryTotalPaidCarnet').textContent = si.totalPaid.toFixed(2);
...
function sendWhatsAppMessage() {
    ...
    if (summaryContainer && whatsappTextarea && sendButton && prevButton) {
        summaryContainer.style.display = 'none'; 
        whatsappTextarea.style.display = 'none'; 
        sendButton.style.display = 'none';     
        prevButton.style.display = 'none';     

        let feedbackDiv = document.getElementById('onScreenFeedbackMessage');
...
function restartSimulation() {
    ...
    // Reset da seleção de data na Tela 1
    document.getElementById('selectedPaymentDateHidden').value = '';
    const paymentDateDetailsDiv = document.getElementById('paymentDateDetails');
    if (paymentDateDetailsDiv) paymentDateDetailsDiv.style.display = 'none';
    document.querySelectorAll('#paymentDayOptionsContainer .option-item.selected').forEach(el => el.classList.remove('selected'));
    document.querySelectorAll('#paymentDayOptionsContainer input[type="radio"]').forEach(radio => radio.checked = false);
    document.getElementById('firstInstallmentDateInfo').textContent = 'A definir'; // Reset visual
    document.getElementById('daysToFirstInstallment').textContent = 'A definir'; // Reset visual
    ...
    selectEntryOption('none', document.querySelector('.entry-options .option-item[onclick*="\'none\'"]'));
    ...
    selectWarrantyOption('none', document.querySelector('.warranty-options .option-item[onclick*="\'none\'"]'));
    ...
    initializeScreen1(); // Isso irá chamar generatePaymentDayOptions e resetar outros campos da tela 1
    showScreen(1);
}
